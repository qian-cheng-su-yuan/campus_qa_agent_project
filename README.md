...
1. 项目背景：学生经常不知道办事流程，人工咨询效率低；
2. 系统方案：把校园制度、通知、FAQ 建成知识库；
3. 核心链路：问题输入后先检索相关文档，再调用大模型生成；
4. 工程拆分：FastAPI 做接口，Streamlit 做演示，业务逻辑放在 services；
5. 风险控制：Prompt 要求模型优先依据知识库，资料不足时说明无法确定；
6. 可扩展点：后续可接入数据库、统一身份认证、微信小程序、管理后台。

## 8. 项目边界说明

为了便于本科生和实习面试演示，本项目没有强制依赖大型向量数据库。当前检索层使用轻量级本地文本相似度实现，便于快速启动。后续如果进入企业项目，可以替换为 Chroma、FAISS、Milvus、Elasticsearch 等组件。

## 9. 简历写法参考

```text
校园智能问答助手｜个人项目｜本地可演示
技术栈：Python、FastAPI、Streamlit、RAG、Prompt、OpenAI-Compatible API、PyCharm
项目描述：面向校园服务咨询场景，整理宿舍、考试、请假、就业、图书馆等资料，构建校园知识库问答助手，帮助学生快速查询校园办事流程和常见问题。
- 使用 FastAPI 设计问答、文档上传、知识库重建等接口，使用 Streamlit 搭建本地演示页面。
- 实现校园资料读取、文本清洗、分段和相似度检索，完成问题到参考片段的召回流程。
- 基于 RAG 实现“用户问题 → 知识库检索 → 上下文拼接 → 大模型回答”的完整链路。
- 设计 Prompt 约束模型优先根据知识库资料回答，资料不足时提示无法确定，降低幻觉风险。
- 增加文档总结、关键词提取、办事清单生成和问答日志，便于调试和面试展示。
```
